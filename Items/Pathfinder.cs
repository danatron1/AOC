...
namespace AOC.Items.Pathfinder
{
    public class Pathfinder<TNode, TMap> where TNode : notnull, IPathfinderNode<TNode>
    {
        ...
        /// <summary>
        /// Takes two ActiveNode inputs (FROM and TO) and outputs an int <br/>
        /// Indicates the cost of stepping onto the new TO tile. Typically <c>from.Cost + something</c> <br/>
        /// <example>
        /// E.g. [Default] If you want Cost to count steps, the new cost should be <c>(from, _) => from.Cost + 1</c> <br/>
        /// If you have an integer map for step penalty, then use <c>(from, to) => from.Cost + pathfinder.Map(to.Node)</c>
        /// </example>
        /// </summary>
        public CostPenaltyDelegate CostPenalty = (from, _) => from.Cost + 1;

        public delegate string MemoryDelegate(ActiveNode from, ActiveNode to);
        /// <summary>
        /// Takes two ActiveNode inputs (FROM and TO) and outputs a string <br/>
        ...
        public bool FindPath(out ActiveNode found)
        {
            found = null;
